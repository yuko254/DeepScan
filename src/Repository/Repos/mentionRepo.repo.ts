...
export class MentionRepo extends BaseRepository<typeof prisma.mentions> {
  ...
  async findByUser(mentioned_user_id: string) {
  ...
  async findByTarget(mention_target_id: string) {
    return this.model.findMany({
  ...
  async createMention(mentioned_user_id: string, mention_target_id: string) {
