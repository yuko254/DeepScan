...
export class CityRepo extends BaseRepository<typeof prisma.cities> {
  ...
  async findByCountry(country_id: bigint): Promise<cities[]> {
    return prisma.cities.findMany({
      where: { country_id },
      orderBy: { name: 'asc' },
    });
  ...
  async findByNameAndCountry(name: string, country_id: bigint): Promise<cities | null> {
    return prisma.cities.findUnique({ where: { name_country_id: { name, country_id } } });
  }
}
